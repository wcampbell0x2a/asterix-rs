...
fn it_works() {
    ...
    if let AsterixMessage::Cat48(ref message) = packet.message {
        ...
        let measured_position_in_polar_coordinates = message.measured_position_in_polar_coordinates.as_ref().unwrap();
        ...
        let mode_3_a_code_in_octal_representation = message.mode_3_a_code_in_octal_representation.as_ref().unwrap();
        ...
        assert_eq!(mode_3_a_code_in_octal_representation.l, L::Mode3CodeDerivedFromTheReplyOfTheTransponder);
        ...
        assert_eq!(mode_smb_data.mb_data, vec![MBData { data: [0xc0, 0x78, 0x00, 0x31, 0xbc, 0x00, 0x00].to_vec() }]);
        ...
        let communications_capability_flight_status = message.communications_capability_flight_status.as_ref().unwrap();
        assert_eq!(communications_capability_flight_status.com, COM::CommACommB);
        assert_eq!(communications_capability_flight_status.stat, STAT::NoAlertNoSPIAircraftAirborne);
